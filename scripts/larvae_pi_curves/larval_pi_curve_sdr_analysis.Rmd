...
```{r, warning=FALSE, message=FALSE}
pi_data<-read.csv("output/larvae_pi_curves/calculated_normalized_photo_rates.csv") #load data
```
...
```{r}
pi.temps<-read.csv("output/larvae_pi_curves/runs_temp.csv")
...
ggsave("figures/larvae_pi_curves/pi_means.png", pi_plot1, dpi=300, w=7, h=5, units="in")
```
...
ggsave("figures/larvae_pi_curves/outliersbygroup.png", outlier.plot, dpi=300, w=8, h=8, units="in")
...
ggsave("figures/larvae_pi_curves/removed_outliersbygroup.png", outlier.plot2, dpi=300, w=8, h=8, units="in")
```
...
augmented <- pi_data %>% 
  nest(-species) %>% 
  mutate(
    fit = map(data, ~ nls(Pc ~ (Am*((AQY*PAR)/(sqrt(Am^2 + (AQY*PAR)^2)))-Rd), data=., start=list(Am=0.5,  AQY=0.001, Rd=.01))),
...
ggsave("figures/larvae_pi_curves/nls_curves_all.png", width=6, height=6, dpi=500)
```
...
ggsave("figures/larvae_pi_curves/parameters.png", metrics_plot, width=10, height=6, dpi=300)
```
