...
```{r}
run_metadata<-read.csv("data/larvae_pi_curves/PI_curves_SDR_Run_Info.csv")
```
...
Sample.Info <- read_csv(file = "data/larvae_pi_curves/PI_curves_SDR_Sample_Info.csv")%>%
  select(!Notes)
...
Run.Info <- read_csv(file = "data/larvae_pi_curves/PI_curves_SDR_Run_Info.csv")
...
for(file in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("output/larvae_pi_curves/PhotosynthesisPlots/",date[file], "_Plate",plate[file],"_",rename[j],"_light", Light_Values[i],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
write.csv(Temp.P, paste0("output/larvae_pi_curves/runs_temp.csv")) #save respiration rate data
```
...
write.csv(Photo.R, paste0("output/larvae_pi_curves/larval_pi_curves_sdr_rates_raw.csv")) #save respiration rate data
...
```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "output/larvae_pi_curves/larval_pi_curves_sdr_rates_raw.csv") #read file back in so slopes don't have to be generated every time
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("output/larvae_pi_curves/calculated_normalized_photo_rates.csv")) #save final file
```
