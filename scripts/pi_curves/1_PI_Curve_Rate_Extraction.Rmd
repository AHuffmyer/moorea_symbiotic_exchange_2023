...
## Import metadata
```{r, warning = FALSE}
path.p <- "data/pi_curves/test/" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/pi_curves/pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/pi_curves/pi_curves_run_metadata_TEST.csv")

# Join all coral and run metadata
metadata <- full_join(sample.info, run.info) %>%
  mutate(Date = as_date(as.character(Date), format = "%Y%m%d", tz = "Pacific/Tahiti"))

# Select only certain columns
metadata <- metadata %>%
  select(sample_id, Species, Lifestage, Run, Temp.Cat, Chamber.Vol.L, Date, Start.time, Stop.time, Light_Value, Light_Level)
```
...
df <- df %>%
  dplyr::mutate(data0 = map(data0, ~select(., Time, Value, Temp)))%>%
  dplyr::mutate(data0 = map(data0, ~(.x %>% filter(complete.cases(.))))) #remove NAs 
...
df <- df %>%
  mutate(intervals = map2(data0, info, function(.x, .y) {
    split(.x, f = cut(as.numeric(.x$Time), breaks = as.numeric(c(.y$Start.time, last(.y$Stop.time))),
                      labels = as.character(.y$Light_Value)))})) %>%
  mutate(data = map(intervals, ~ unnest_wider(.x, names_sep = "_", values_fn = list)))
```


ANOTHER TEST, DOESN'T WORK
```{r}
#this is stopping at the next start time, not the appropriate stop time 

df <- df %>%
  mutate(intervals = map2(data0, info, function(.x, .y) {
    breaks <- c(.y$Start.time, .y$Stop.time)
    labels <- as.character(.y$Light_Value)
    
    # Ensure that labels have the same length as breaks
    if (length(labels) < length(breaks)) {
      labels <- rep(labels, length.out = length(breaks))
    }
    
    # Create a new column for Light_Value based on the intervals
    .x$Light_Value <- cut(as.numeric(.x$Time), breaks = as.numeric(breaks), labels = labels)
    return(.x)
  })) %>%
...
df <- df %>%
  dplyr::mutate(thin_data = map(data, ~ slice(., seq(1, nrow(.), thin_par))))
...
#select only the essential columns
mx <- select(metadata, Species, sample_id, Run, Chamber.Vol.L, Temp.Cat)
nrow(mx)
nrow(distinct(df))
...
write.csv(pr, "output/pi_curves/pi_curve_extracted_rates_Raw.csv")
```
...
sa <- read.csv("output/physiology/surface_area.csv")
...
pr.out <- pr %>% 
  select(Species, sample_id, Light_Value.x, Temp.Cat.x, Run, micromol.cm2.s, micromol.cm2.h)
length(unique(pr.out$sample_id))

# Write to output file
write.csv(pr.out, "output/pi_curves/pi_curve_extracted_rates.csv")
```
