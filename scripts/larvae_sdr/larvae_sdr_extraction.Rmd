---
title: "Larval Respiration and Photosynthesis Rates Extraction with LoLinR - Moorea 2023"
author: "AS Huffmyer"
date: '2023'
output:
  html_document:
...
```{r, warning=FALSE, message=FALSE}
path.p<-"data/larvae_sdr/runs" #location of files
```
...
```{r}
run_metadata<-read.csv("data/larvae_sdr/larvae_SDR_Run_Info.csv")
```
...
Sample.Info <- read_csv(file = "data/larvae_sdr/larvae_SDR_Sample_Info.csv")%>%
  select(!Notes)

rename <- Sample.Info$Chamber.ID
samp <- Sample.Info$Sample.ID

plate <- as.numeric(str_sub(file.names, 18, 19)) #EDIT THIS AS NEEDED 

date <- str_sub(file.names, 4, str_length(file.names)-31) #grab date from file name EDIT IF NEEDED

# Load PI curve run metadata (i.e., light levels and interval times for each run)
Run.Info <- read_csv(file = "data/larvae_sdr/larvae_SDR_Run_Info.csv")
...
for(file in 1:length(file.names)) { # for every file in list start at the first and run this following function
  ...
  for(j in 1:(ncol(Photo.Data.N))){
    ...
    pdf(paste0("output/larvae_sdr/PhotosynthesisPlots/",date[file], "_Plate",plate[file],"_",rename[j],"_light", Light_Values[i],"_regression_trunc.pdf")) #generate output file names
    plot(model)
    dev.off()
...
write.csv(Temp.P, paste0("output/larvae_sdr/runs_temp.csv")) #save respiration rate data
```
...
write.csv(Photo.R, paste0("output/larvae_sdr/larvae_sdr_rates_raw.csv")) #save respiration rate data
...
```{r, warning=FALSE, message=FALSE}
Photo.Rates <- read.csv(file = "output/larvae_sdr/larvae_sdr_rates_raw.csv") #read file back in so slopes don't have to be generated every time
...
oxygen$Species<-Sample.Info$Species[match(oxygen$unique, Sample.Info$unique)]
...
plot(blank_data$P.umol.min~as.factor(blank_data$code), xlab="Plate", ylab="umol O2 min-1") #blanks during dark phase
...
Save as .csv file.  
```{r, warning=FALSE, message=FALSE}
write.csv(oxygen.bio, paste0("output/larvae_sdr/larvae_calculated_normalized_photo_rates.csv")) #save final file
```
