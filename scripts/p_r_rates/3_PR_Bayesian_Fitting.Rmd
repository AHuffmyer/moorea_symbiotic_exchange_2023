---
title: "Bayesian PI Curve fitting"
...
All <- left_join(Data, md, by="sample_id")
```
...
```{r}
names <- unique(Data_sub$sample_id)
names 
fits <- setNames(vector("list", length(names)), names)

for (i in names) {
  data_model<-Data_sub %>% filter(sample_id==i)
...
param.estimates <- param.estimates %>% separate(variable,  into=c("sample_id", "metric"), sep=7, remove = FALSE)
param.estimates <- param.estimates %>% separate(sample_id, into = c('species', 'sample_id'), sep=4, remove = FALSE)
param.estimates$sample_id <- paste0(param.estimates$species, param.estimates$sample_id)
param.estimates$sample_id <- gsub("\\.","-", param.estimates$sample_id)
...
```{r}
Data.out <- left_join(param.estimates, md, by="sample_id")
```
...
ggsave("output/p_r_rates/pr_bayes_estimates.pdf",estimate.plots,  width = 6, height=6)
```
