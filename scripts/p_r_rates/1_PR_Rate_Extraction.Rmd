...
run.info <- read_csv(file = "data/p_r_rates/pr_run_metadata.csv")
...
LEDR %>% ggplot(aes(x=as.numeric(Temp.Cat.x), y=micromol.cm2.h, colour = Species))+
  geom_point()+
  facet_wrap(vars(Species, Lifestage), nrow=3)+
  labs(x = "Temp °C")+
  labs(y = "Rdark (micromol.cm2.h)")

ggsave("output/Rdark.pdf", width = 4, height = 4)
```
...
Pmax %>% ggplot(aes(x=as.numeric(Temp.Cat.x), y=micromol.cm2.h, colour = Species))+
  geom_point()+
  facet_wrap(vars(Species, Lifestage), nrow=3)+
  labs(x = "Temp °C")+
  labs(y = "Pmax (micromol.cm2.h)")

ggsave("output/Pmax.pdf", width = 4, height = 4)
