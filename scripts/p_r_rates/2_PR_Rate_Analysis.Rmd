...
pairs(emm)
```
P max is lower in POR recruits than adults. No difference in other species. 
...
pairs(emm)
```
LEDR is higher in POR adults than recruits but not different between other species. 
...
```{r}
model3<-aov(P~LEDR*Species, data=pr_wide)
summary(model3)
```
The relationship between P and LEDR is strong and the relationship is different between species. 
...
pairs(emm)
```
Effect of species and lifestage on P normalized to per cell rates. Recruits look higher than adults. Particularly in POC. No strong temperature effects. Species x lifestage is significant - the difference between lifestages is different by species. 
