...
```{r}
#plot  values
pr_ratio_plot<-pr_wide %>% filter(PR_ratio>0) %>% 
...
pairs(emm)
```
Significant species x lifestage interaction. No effect of temperature. P gross is lower in POR recruits than adults. No difference in other species. 
...
LEDR
```{r}
ledr_plot_recruit<-pr_wide %>% filter(Lifestage=="Recruit") %>% 
...
Doesn't look like a strong relationship. 
...
```{r}
model5<-aov(Pgross~LEDR*Species, data=pr_wide)
summary(model5)
...
pairs(emm)
```
Interaction of species and lifestage, almost an effect of temp but not quite. Recruit Pgross per cell is higher in POC than adults. No difference in other species. 
...
emm<-emmeans(model8, ~Lifestage)
pairs(emm)
```
There is a lifestage effect. Recruit Pnet per cell is higher than adults. This is most apparent in POC as we saw in Pgross per cell. 
