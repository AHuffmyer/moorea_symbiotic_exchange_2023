...
```{r} 
#specify data 
Data<-read_csv(here("output/p_r_rates","pr_extracted_rates.csv"))

Data$PAR <- as.numeric(Data$Light_Value.x) 
Data$Pc <- as.numeric(Data$micromol.cm2.s) 

Data %>% 
ggplot(aes(x=PAR, y=Pc, color=species, fill=lifestage))+ 
  geom_point() 

raw.plot <- Data %>% 
ggplot(aes(x=PAR, y=Pc, color=species, group=sample_id, fill=lifestage))+ 
  geom_point()+
  geom_line()+
  facet_wrap(~ Species*Temp.Cat.x,  ncol = 10)+
  theme(legend.position = "none")

ggsave("figures/p_r_rates/pr_raw_plots.pdf",raw.plot,  width = 10, height=6)

ests <- Data%>%
  group_by(species, Temp.Cat.x, sample_id)%>%
  summarise(Rd= min(Pc),
            Pmax = max(Pc))
...
ggplot(aes(x=Temp.Cat.x, y=-Rd, color=species, group=species, fill=lifestage))+ 
  geom_point()+
  #geom_line()+
  facet_wrap(~ species)+
  theme(legend.position = "none")
...
ggplot(aes(x=Temp.Cat.x, y=Pmax, color=species, group=species, fill=lifestage))+ 
  geom_point()+
  #geom_line()+
  facet_wrap(~ species)+
  theme(legend.position = "none")
...
#rd= dark respiration                  
  ...
  write_csv(nls_data, "output/p_r_rates/pr_pars_nls.csv") 
``` 
...
```{r} 
pars <- nls_data 
md <- read_csv("data/p_r_rates/pr_coral_metadata.csv") 
...
jpeg("output/pi_curves/PI_metrics_nls.jpg") 
PI.params <- df %>% 
...
pars <- read_csv("output/p_r_rates/pr_pars_nls.csv") 
md <- read_csv("data/p_r_rates/pr_coral_metadata.csv") 
df <- left_join(pars, md) 
...
ggsave("output/p_r_rates/Pmax_outliersbygroup.pdf", outlier.plot, dpi=300, w=16, h=8, units="in") 
