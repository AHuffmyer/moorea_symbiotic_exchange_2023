...
ERROR HERE - FIX "permanova_results"
```{r}
...
ggsave("figures/isotopes/glycolysis-pathway-strategy.png", width=7, height=5)
```
...
ggsave("figures/isotopes/gogat-pathway-strategy.png", width=7, height=5)
```


# Strategy plot for TCA pathway 
...
```{r}
tca_centroid <- all_data %>%
  filter(Compound %in% tca_mets) %>%
  droplevels() %>%
  group_by(sample, species, lifestage, treatment) %>%
  summarise(
    pathway_conc   = mean(log.intensity.norm, na.rm = TRUE),
...
Try centroid method. 
```{r}
tca_group_centroid <- tca_centroid %>%
  group_by(species, lifestage, treatment) %>%
  summarise(
    mean_conc   = mean(pathway_conc, na.rm = TRUE),
    mean_enrich = mean(pathway_enrich, na.rm = TRUE),
...
ggplot() +
  ...
  theme_classic() +
  labs(
    x = "TCA pathway concentration",
    y = "TCA pathway 13C enrichment", 
    title = "TCA Pathway"
  )

ggsave("figures/isotopes/tca-pathway-strategy.png", width=7, height=5)
```


# Strategy plot for energy cofactors  
...
```{r}
energy_centroid <- all_data %>%
  filter(Compound %in% energy_mets) %>%
  droplevels() %>%
  group_by(sample, species, lifestage, treatment) %>%
  summarise(
    pathway_conc   = mean(log.intensity.norm, na.rm = TRUE),
...
Try centroid method. 
```{r}
energy_group_centroid <- energy_centroid %>%
  group_by(species, lifestage, treatment) %>%
  summarise(
    mean_conc   = mean(pathway_conc, na.rm = TRUE),
    mean_enrich = mean(pathway_enrich, na.rm = TRUE),
...
ggplot() +
  ...
  theme_classic() +
  labs(
    x = "Energy cofactors concentration",
    y = "Energy cofactors 13C enrichment", 
    title = "Energy Cofactors"
  )

ggsave("figures/isotopes/energy-pathway-strategy.png", width=7, height=5)
```
