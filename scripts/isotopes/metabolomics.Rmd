...
# Strategy plot for Arg-gln pathway 
...
```{r}
dipep_centroid <- all_data %>%
  filter(Compound %in% dipep_mets) %>%
  droplevels() %>%
  group_by(sample, species, lifestage, treatment) %>%
  summarise(
    pathway_conc   = mean(log.intensity.norm, na.rm = TRUE),
...
Try centroid method. 
```{r}
dipep_group_centroid <- dipep_centroid %>%
  group_by(species, lifestage, treatment) %>%
  summarise(
    mean_conc   = mean(pathway_conc, na.rm = TRUE),
    mean_enrich = mean(pathway_enrich, na.rm = TRUE),
...
ggplot() +
  ...
  theme_classic() +
  labs(
    x = "Dipeptides concentration",
    y = "Dipeptides 13C enrichment", 
    title = "Dipeptides"
  )

ggsave("figures/isotopes/dipeptides-pathway-strategy.png", width=7, height=5)
```
