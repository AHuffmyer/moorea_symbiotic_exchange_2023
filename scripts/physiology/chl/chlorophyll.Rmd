...
We are missing chl data for POC-A6, POC-A17, ACR-A15, and POR-R11.
4:17
ACR-R11 is one of the two that I put a chl tube aside for already in Box S2
4:18
I also noticed that POR-A9 has 4 readings where as all the others had 2. Are two of those maybe a typo?
4:19
Same is true for POC-R11 and POC-A16, which may be typos of other colonies.
...
model1<-chl%>%
  ...
  aov(total.ugchl.cell~species*lifestage*Temp.Cat, data=.)
...
pairs(emm)
```
Species x lifestage effects. POR recruits are lower then adults for chl per symbiont cell. 
